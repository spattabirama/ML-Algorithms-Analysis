...
'''def numerical_label_subsets(att, y_train):
    sorted_att_values = np.sort(att)
    mean_att_values = []
    for i in range(len(sorted_att_values)-1):
        mean_att_values.append((sorted_att_values[i] + sorted_att_values[i+1])/2)
    subsets = {}
    for val in mean_att_values:
        subsets[val] = {}
        subsets[val]["<="] = []
        subsets[val][">"] = []
        for i in range(len(att)):
