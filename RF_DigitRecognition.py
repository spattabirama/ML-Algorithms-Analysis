...
def evaluate_random_forest(n_trees, test, class_column_name, df_type):
    ...
    pm.precision = metrics.precision_score(actual, max_prediction, average='macro')
    pm.recall = metrics.recall_score(actual, max_prediction, average='macro')
    pm.f_score = metrics.f1_score(actual, max_prediction, average='macro')
...
def load_dataset():
    ...
    data = EncodeOrNormalizing(df)
    return data


def EncodeOrNormalizing(data):
    for column in data.columns[:-1]:
        if not (data[column] == 0).all():
            normdata = Normalizationdf(data[column])
            data[column] = normdata
    return data
...
def main():
    ...
    min_size_for_split = round(0.03 * len(dataset.index))
