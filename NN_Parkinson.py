...
def preprocess(x):
    for i in range(x.shape[1]):
        column = x[:, i]
        x[:, i] = (column - np.min(column)) / (np.max(column) - np.min(column))
    return x
...
def one_hot_encode(arr, unique_vals):
    encoder = OneHotEncoder(categories=[unique_vals])
    arr_2d = arr.reshape(-1, 1)
    one_hot_encoded = encoder.fit_transform(arr_2d)
...
def cost_verification():
    ...
    n_att = x.shape[1]
    neurons_per_layer = [n_att, 20, 8, n_cls]
...
def main():
    ...
    print("Lambda:", lamda)
    print("Alpha:", alpha)
    print("k_stop_criterion:", k_stop_criterion)
